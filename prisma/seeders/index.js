...
const main = async () => {
  ...
  if (commandLineArguments?.[1]?.includes('--matches')) {
    const [, value] = commandLineArguments[1].split('=');

    if (value) {
      seeders = seeders.filter(file => file.toLowerCase().includes(value.toLowerCase()));
    }
  }
